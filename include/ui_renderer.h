...
public:
    ...
    // Main render calls
    void renderSpectrum(const std::vector<float>& magnitude, uint32_t center_freq, uint32_t span, uint32_t sample_rate);
    void renderWaterfall(const std::vector<float>& magnitude);
    void renderStatusBar(uint32_t freq, uint32_t span, int gain, const char* freq_input = nullptr);
...
private:
    ...
    // Helpers
    void drawSpectrumGraph(const std::vector<float>& mag, int x, int y, int w, int h);
